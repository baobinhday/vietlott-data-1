...
class InverseHybridTrioStrategy(PredictModel):
    ...
    def __init__(self, df: pd.DataFrame, steiner: SteinerStrategy, top_k: int = 15, time_predict: int = 6):
        ...
        self.strat_cold = InverseHybridStrategy(
            proposer=ColdNumbersStrategy(df, time_predict=2, min_val=self.min_val, max_val=self.max_val, lookback_days=365, selection_weight=0.7),
            steiner=steiner,
            top_k=top_k,
            coverage=2,
            time_predict=2
        ...
        self.strat_pair = InverseHybridStrategy(
            proposer=PairFrequencyStrategy(df, time_predict=2, min_val=self.min_val, max_val=self.max_val, lookback_days=365),
            steiner=steiner,
            top_k=top_k,
            coverage=2,
            time_predict=2
        ...
        self.strat_pattern = InverseHybridStrategy(
            proposer=PatternStrategy(df, time_predict=2, min_val=self.min_val, max_val=self.max_val, lookback_days=180, pattern_weight=0.6),
            steiner=steiner,
            top_k=top_k,
            coverage=2,
            time_predict=2
